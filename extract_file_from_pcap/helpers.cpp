...
int memwildcardcmp(const void *s1, const void *s2, size_t n, int caseSensitive)
{
	if (n != 0)
		{
		register const unsigned char	*p1 = (unsigned char*)s1, *p2 = (unsigned char*)s2;
		do
			{
...
unsigned short htos(unsigned char s[], int endian)
{
	...
	char			temp = 'x';
	bytes = (unsigned char	*)memcpy(bytes, s, sizeof(short));
...
unsigned int htoi(unsigned char s[], int endian)
{
	...
	bytes = (unsigned char	*)memcpy(bytes, s, length);

...
void usage(void)
{
	...
	fprintf(stderr, "-p  - set input file type is .pcap. %s", NEWLINE);
...
void process_command_line(int argc, char **argv, f_state *s)
{
	...
	while ((i = getopt(argc, argv, "o:b:c:t:s:i:k:hqmQTadvVw:p")) != -1)
		{
		switch (i)
			{
			...
			case 'p':
				set_input_file_type(s, INPUT_FILE_TYPE_PCAP);
				printf("extract from pcap file\n");
				break;
...
int main(int argc, char **argv)
{
	...
	for (++argv; *argv != NULL && strcmp(*argv++,"-i"); );
	...
	for (; *argv != NULL; ++argv)
	{
		...
		if (testFile)
		{
			...
			audit_msg(s, "\ninput file : %s\n", *argv);
			if (INPUT_FILE_TYPE_PCAP == get_input_file_type(s)) {
				pcap_process_file(s);
			} else {
				process_file(s);
			}
